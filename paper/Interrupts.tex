...
\caption{Interrupt line(INTR) gets set high. The CPU finishes the current instruction and performs a context switch.}
\end{center}
\end{figure}
...
\caption{The non-maskable interrupt (hardware) handler is invoked by the CPU by looking up the handler address from the
